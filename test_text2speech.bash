...
if [ ! -f $TEXT2SPEECHPROG ];then
   echo "ERROR: The $TEXT2SPEECHPROG program seems not to be installed on your RPI"
   echo "Hint: install it by doing 'sudo apt-get install alsa-utils' first"
   apt-cache search pico2wave  # find package for the program 'pico2wave'
   exit 1
fi
...
echo "$INTROTEXT"
$TEXT2SPEECHPROG -w $SOUNDFILE "$INTROTEXT"
aplay $SOUNDFILE
...
do
   ...
   $TEXT2SPEECHPROG -w $SOUNDFILE "$ANS"        # create the sound file
