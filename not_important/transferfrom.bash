#!/bin/bash
#
# Script status: Yet to be tested
#
...
if [ -d $(pwd)/${SUBDIR} ];then
   ...
   echo;echo "Please backup and the remove or move your local $(pwd)/${SUBDIR} directory"
   ...
   if [ "$(echo "$SUBDIR" | cut -c 1)" = "/" ];then
      ...
      if [ -d ${HOME}/${SUBDIR} ];then
         echo "You have already a directory called ${HOME}/${SUBDIR}"
         echo "I will not destroy it. Now exit"
         exit 1
      fi
      mkdir -p ${HOME}/${SUBDIR}
