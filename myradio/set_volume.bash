...
# 
# Programs sets local or remote volume on RPI
#
# Only initial values:
IP=""
CONTRL=Master
...
usage_exit() {
   ...
   echo "Ex1: $(basename $0)      # set remote volume"
   echo "Ex2: $(basename $0) 75   # set local volume to 75%"
   exit 1
}
...
setvolumblackboxRPI() {
   ...
   if [ ! -z "$SCONTROL" ];then
      if [ $DEBUG -eq 0 ];then
         ...
         amixer get $SCONTROL >/dev/null 2>&1
      else
